...
def get__wav(
    ...
    # 解决并发问题
    hz, max_sec, t2s_model, config = get_gpt_weights(gpt_model_path)
    vq_model, hps = get_sovits_weights(sovits_model_path)
...
# get_sovits_weights(sovits_path)
# get_gpt_weights(gpt_path)
...
async def _endpoint(request: Request):
    json_post_raw = await request.json()
    return handle(
        json_post_raw.get("gpt_model_path"),
        json_post_raw.get("sovits_model_path"),
        json_post_raw.get("refer_wav_path"),
        json_post_raw.get("prompt_text"),
        json_post_raw.get("prompt_language"),
