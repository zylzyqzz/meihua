...
class Avatar:
    ...
    def inference(self, audio_path, out_vid_name, fps, skip_save_images):
        ...
        if out_vid_name is not None and args.skip_save_images is False:
            ...
            frame_pattern = os.path.join(self.avatar_path, "tmp", "%08d.png")
            subprocess.run([
                "ffmpeg", "-y", "-v", "warning", "-r", str(fps),
                "-f", "image2", "-i", frame_pattern, "-vcodec", "libx264",
                "-vf", "format=yuv420p", "-crf", "18", temp_video,
            ], check=True)

            output_vid = os.path.join(self.video_out_path, out_vid_name + ".mp4")
            if args.silent_output:
                os.replace(temp_video, output_vid)
            else:
                subprocess.run([
                    "ffmpeg", "-y", "-v", "warning", "-i", temp_video,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prepare_only",
        action="store_true",
        help="Create reusable avatar caches without rendering a test audio clip",
    )
    parser.add_argument(
        "--silent_output",
        action="store_true",
        help="Write a silent lip-sync video; the host application owns audio playback",
    )
    ...
    # Avatar preparation only needs the VAE. Avoid loading the 3.4 GB UNet and
    # Whisper model until the user explicitly requests a lip-sync preview.
    runtime_dtype = torch.float16 if device.type == "cuda" else torch.float32
    if args.prepare_only:
        vae = VAE(model_path=os.path.join("models", args.vae_type))
        vae.vae = vae.vae.to(device=device, dtype=runtime_dtype)
        unet = None
        pe = None
        timesteps = None
        ...
        pe = pe.to(device=device, dtype=runtime_dtype)
        vae.vae = vae.vae.to(device=device, dtype=runtime_dtype)
        unet.model = unet.model.to(device=device, dtype=runtime_dtype)

        # Initialize audio processor and Whisper model only for real renders.
