...
import { giftGlyph, giftIconUrl, referenceGiftCatalog, type GiftCatalogEntry } from './giftCatalog.js';
...
const tabs: Array<{ id: Tab; label: string; note: string }> = [
  ...
  { id: 'simple', label: '资格与队列', note: '点赞、评论和礼物规则' },
...
const sourceNames: Record<ObsSourceId, string> = {
  sticker: '自定义贴纸', avatar: '数字人画面', background: '直播背景', 'current-viewer': '当前测算用户（兼容）', hexagram: '卦象与当前测算', subtitles: '口播字幕', queue: '资格与排队名单',
...
function PresentationPanel({ settings, assets, onSaved }: { settings: AppSettings; assets: MediaAsset[]; onSaved: () => Promise<void> }) {
  ...
  const upload = async () => {
    ...
    try {
      ...
      const profile = await request<{ profile: AppSettings['presentation']['profiles'][number] }>('/api/presentation/videos', { method: 'POST', body: JSON.stringify({ name: videoName.trim() || videoFile.name, assetId: asset.id, playback: 'LOOP', fit: 'COVER' }) });
  ...
  return <Panel title="画面模式 · 统一播报" hint="默认用预录视频保证链路稳定；只有选择实时数字人时才会调用 MuseTalk 或云端数字人。">
...
export function App() {
  ...
  const changeCommentKeywords = (value: string) => settings && setSettings({ ...settings, engagement: { ...settings.engagement, commentRules: [{ ...(firstComment ?? { id: 'simple-comment', label: '评论排队', priority: 'NORMAL', matchMode: 'CONTAINS', stripKeyword: true, cooldownMinutes: 10, queueExpireMinutes: settings.queue.expireMinutes, speechTargetSeconds: settings.reading.speechTargetSeconds }), enabled: true, keywords: [...new Set(value.split(/[、,，]+/).map((item) => item.trim()).filter(Boolean))], queueExpireMinutes: settings.queue.expireMinutes, speechTargetSeconds: settings.reading.speechTargetSeconds }] } });

  const saveSimpleRules = async () => {
    if (!settings) return;
    const normalized: AppSettings = {
      ...settings,
      gifts: { ...settings.gifts, rules: settings.gifts.rules.map((rule) => ({ ...rule, enabled: true, minRepeatCount: 1, priority: 'NORMAL', requireStreakEnd: false, leaderboardPoints: Math.max(1, Math.round(rule.speechTargetSeconds / 20)) })) },
      engagement: {
        ...settings.engagement,
        likeRules: settings.engagement.likeRules.slice(0, 1).map((rule) => ({ ...rule, enabled: true, priority: 'NORMAL', speechTargetSeconds: settings.reading.speechTargetSeconds })),
        commentRules: settings.engagement.commentRules.slice(0, 1).map((rule) => ({ ...rule, enabled: true, priority: 'NORMAL', matchMode: 'CONTAINS', queueExpireMinutes: settings.queue.expireMinutes, speechTargetSeconds: settings.reading.speechTargetSeconds })),
      },
    };
  ...
  const capturedGiftEntries: GiftCatalogEntry[] = capturedGifts.map((gift) => ({
    ...
    name: gift.giftName, coins: gift.coinValue, giftId: gift.giftId, source: 'CAPTURED',
    iconUrl: giftIconUrl(gift.giftId, gift.giftName), verification: 'TIKFINITY_CAPTURED',
  }));
  ...
  const simplePage = settings && <section className="page simple-page">
    <div className="workspace-title"><div><span className="eyebrow">QUEUE AUTOMATION</span><h2>资格与队列</h2><p>只设置触发条件，去重、资格、排序、过期和插队由系统自动完成。</p></div><button className="primary" onClick={() => void act('save-rules', saveSimpleRules, '排队规则已保存', true)}><CheckCircle weight="fill" />保存全部规则</button></div>
    <div className="qualification-summary">
      ...
      <div><ChatCircle weight="fill" /><span>评论关键词<small>命中后等待问题</small></span><strong>{firstComment?.keywords.length ?? 0} 个</strong></div>
      <div><Timer weight="fill" /><span>免费排队<small>超时自动取消</small></span><strong>{settings.queue.expireMinutes} 分钟</strong></div>
      <div><Gift weight="fill" /><span>礼物规则<small>礼物用户永久等待</small></span><strong>{settings.gifts.rules.length} 条</strong></div>
    </div>
    <div className="qualification-layout">
      <Panel title="免费资格" hint="观众满足其中一项即可进入等待链路。" className="free-rules-panel"><div className="simple-rules"><label><span><Heart weight="fill" />点赞资格</span><input type="number" min="1" value={firstLike?.threshold ?? 100} onChange={(event) => changeLikeThreshold(Number(event.target.value))} /><small>同一观众累计达到次数后发放一次资格，不重复计算房间总点赞。</small></label><label><span><ChatCircle weight="fill" />评论资格</span><input value={firstComment?.keywords.join('、') ?? ''} onChange={(event) => changeCommentKeywords(event.target.value)} /><small>多个关键词请用“、”分隔，数量不限；支持“关键词＋问题”，只发关键词时继续等待下一条有效问题。</small></label></div></Panel>
      <Panel title="排队策略" hint="系统自动处理，不需要直播中手工操作。" className="queue-policy-panel"><div className="policy-fields"><label><span><Timer weight="fill" />免费排队时限</span><input type="number" min="1" value={settings.queue.expireMinutes} onChange={(event) => setSettings({ ...settings, queue: { ...settings.queue, expireMinutes: Math.max(1, Number(event.target.value)) } })} /><small>超过该时间自动取消并要求重新取得资格。</small></label><label><span><SpeakerHigh weight="fill" />免费测算时长</span><select value={settings.reading.speechTargetSeconds} onChange={(event) => setSettings({ ...settings, reading: { ...settings.reading, speechTargetSeconds: Number(event.target.value) } })}>{speechDurationOptions.map((seconds) => <option key={seconds} value={seconds}>{seconds} 秒</option>)}</select><small>文案字数会结合输出语言和语速自动换算。</small></label></div><div className="queue-flow"><b>自动执行顺序</b><span>取得资格 → 提交问题 → 进入队列 → 到期清理或开始测算</span><small>礼物资格不会因为免费排队时限而过期。</small></div></Panel>
    </div>
    ...
    <Panel title="礼物与测算时长" hint="选择礼物并指定播报时长即可；礼物 ID 优先匹配，实际抓到的新礼物会自动进入礼物库。" className="gift-rule-panel" action={<button onClick={() => setGiftLibraryOpen((value) => !value)}><Gift weight="fill" />{giftLibraryOpen ? '收起礼物库' : '添加礼物'}</button>}>
      ...
      <div className="gift-rule-grid">{settings.gifts.rules.map((rule) => { const captured = capturedGifts.find((gift) => gift.giftId && gift.giftId === rule.giftId) ?? capturedGifts.find((gift) => gift.giftName.toLocaleLowerCase() === rule.giftName.toLocaleLowerCase()); return <article key={rule.id} className={rule.enabled ? 'enabled' : ''}>
        ...
        <label>测算时间<select value={rule.speechTargetSeconds} onChange={(event) => setSettings({ ...settings, gifts: { ...settings.gifts, rules: settings.gifts.rules.map((item) => item.id === rule.id ? { ...item, speechTargetSeconds: Number(event.target.value), priority: 'HIGH' } : item) } })}>{speechDurationOptions.map((seconds) => <option key={seconds} value={seconds}>{seconds} 秒</option>)}</select></label>
  ...
  const connectPage = settings && <section className="page connect-page">
    <div className="workspace-title"><div><span className="eyebrow">INTEGRATION HEALTH</span><h2>接入健康</h2><p>配置一次，健康状态只以真实连接与有效事件为准。</p></div><button className="primary" onClick={() => void act('connect-save', () => request('/api/settings', { method: 'PUT', body: JSON.stringify(settings) }), '接口设置已保存', true)}><CheckCircle weight="fill" />保存接口</button></div>
    <div className="integration-overview">
      ...
      <article><UserCircle weight="fill" /><span><b>人物画面</b><small>{settings.providers.avatar.adapter === 'none' ? 'OBS 人物素材源' : settings.providers.avatar.adapter === 'mock' ? '数字人供应商适配层' : 'VTube Studio 实验动作'}</small></span><Pill status={health?.avatar ?? 'UNKNOWN'} /></article>
      <div className="integration-flow"><b>正式链路</b><span>TikFinity 事件 → 内容生成 → 语音合成 → OBS 独立画面</span><small>任一环节未通过真实测试，都不会被标记为 READY。</small></div>
    </div>
    <div className="connect-grid">
      ...
      <article><header><span><Lightning weight="fill" /><b>内容生成</b></span><Pill status={health?.llm ?? 'UNKNOWN'} /></header><p>模型只负责把确定的卦象结论整理成口播；卦象、动爻和结果由本地梅花引擎锁定。接口不可用或返回过短时，自动用同一事实集补齐，不会播出空泛答案。</p><label>方式<select value={settings.providers.llm.adapter} onChange={(event) => patchLlm({ adapter: event.target.value as typeof settings.providers.llm.adapter })}><option value="rule-based">本地规则引擎</option><option value="openai-compatible">OpenAI 兼容接口（阿里百炼 / DeepSeek 等）</option></select></label>{settings.providers.llm.adapter === 'openai-compatible' && <div className="provider-fields"><label>接口地址<input placeholder="https://dashscope.aliyuncs.com/compatible-mode/v1" value={settings.providers.llm.baseUrl} onChange={(event) => patchLlm({ baseUrl: event.target.value })} /></label><label>模型<input placeholder="qwen-plus 或 deepseek-chat" value={settings.providers.llm.model} onChange={(event) => patchLlm({ model: event.target.value })} /></label><label className="wide">API Key<input type="password" autoComplete="new-password" placeholder={secretStatus?.llm.configured ? '已安全保存；留空不会覆盖' : '输入 API Key'} value={llmApiKey} onChange={(event) => setLlmApiKey(event.target.value)} /></label><div className="secret-state"><span>{secretStatus?.llm.configured ? '✓ API Key 已由 Windows DPAPI 加密保存' : '尚未保存 API Key'}</span><div><button disabled={!llmApiKey || Boolean(busy)} onClick={() => void act('llm-secret', () => saveProviderSecret('llm', llmApiKey), '内容生成 API Key 已安全保存')}>保存密钥</button><button className="primary" disabled={Boolean(busy)} onClick={() => void act('llm-test', async () => { await request('/api/settings', { method: 'PUT', body: JSON.stringify(settings) }); await request('/api/providers/llm/test', { method: 'POST' }); }, '外部内容生成已通过真实调用测试', true)}>测试连接</button></div></div></div>}</article>
      ...
      <article><header><span><UserCircle weight="fill" /><b>人物画面</b></span><Pill status={health?.avatar ?? 'UNKNOWN'} /></header><p>推荐使用中控自带 OBS 人物素材源；不依赖 Steam，不做不可靠口型。</p><label>方式<select value={settings.providers.avatar.adapter} onChange={(event) => setSettings({ ...settings, providers: { ...settings.providers, avatar: { ...settings.providers.avatar, adapter: event.target.value as typeof settings.providers.avatar.adapter } } })}><option value="none">OBS 人物素材源（推荐）</option><option value="mock">数字人供应商适配层（模拟）</option><option value="musetalk">MuseTalk 本地实时口型（需 GPU 直播机）</option><option value="vtube-studio">VTube Studio 阶段动作（实验）</option></select></label>{settings.providers.avatar.adapter === 'mock' && <div className="secret-state adapter-layer-note"><span>{health?.avatarProvider?.status === 'READY' ? '适配层完成 · 本地模拟已连接，可用于全链路联动测试' : '适配层完成 · 供应商待接入'}</span><small>真实供应商需完成账户授权、绿幕输出与 OBS 录制验收后，才会标记为已接入。</small></div>}{settings.providers.avatar.adapter === 'musetalk' && <label>渲染服务地址<input value={settings.providers.avatar.url} onChange={(event) => setSettings({ ...settings, providers: { ...settings.providers, avatar: { ...settings.providers.avatar, url: event.target.value } } })} /></label>}{settings.providers.avatar.adapter === 'vtube-studio' && <label>实验接口地址<input value={settings.providers.avatar.url} onChange={(event) => setSettings({ ...settings, providers: { ...settings.providers, avatar: { ...settings.providers.avatar, url: event.target.value } } })} /></label>}<button onClick={() => setTab('obs')}><MonitorPlay />去上传人物素材并预览</button><small className="truth-note">Warudo 目前仅保留协议、没有实现，已从可选项移除；真实供应商能力一律以验收记录为准。</small></article>
    </div>
    <PresentationPanel settings={settings} assets={assets} onSaved={() => refresh(true)} />
    <Panel title="数字人投产方案" hint="先使用稳定、可控、能验收的方式；外部软件不再冒充默认能力。"><div className="avatar-production"><div className="recommended-flow"><span>推荐</span><b>透明 WebM / 绿幕 MP4 人物动作</b><p>上传待机、讲解、礼物感谢三个素材 → OBS 添加 <code>/obs/source/avatar</code> → 中控按同一导演阶段自动切换。人物嘴部遮挡或使用自然循环动作，不需要口型同步。</p></div><div><b>OBS 内的实际图层</b><p>人物源、卦象、当前用户、队列和榜单分别添加。位置、大小、裁剪完全由 OBS 调整。</p></div><div><b>VTube Studio</b><p>仅作为高级实验能力：OBS 直接抓窗口，中控最多触发阶段热键。未授权、无模型时不会显示 READY，也不影响核心测算流程。</p></div></div>{settings.providers.avatar.adapter === 'vtube-studio' && <details className="advanced-connect"><summary>打开 VTube Studio 实验联调</summary><div className="wizard-steps"><button onClick={() => void act('vtube-connect', () => request('/api/avatar/vtube/connect', { method: 'POST' }), '已检查 VTube Studio 连接')}><span>1</span><b>检查连接</b><small>确认 Plugin API 已开启</small></button><button onClick={() => void act('vtube-authorize', () => request('/api/avatar/vtube/authorize', { method: 'POST' }), '请在授权弹窗中确认')}><span>2</span><b>授权</b><small>令牌由 DPAPI 保存</small></button><button onClick={() => void act('vtube-actions', () => request('/api/avatar/vtube/test-actions', { method: 'POST' }), '已发送动作测试')}><span>3</span><b>测试动作</b><small>不发送嘴型参数</small></button></div></details>}</Panel>
  </section>;
