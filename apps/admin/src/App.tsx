  ...
  const livePage = <section className="page dashboard-page">
    ...
    <div className="dashboard-grid">
      <Panel title="节目监看" hint="当前任务的唯一真相源" className="program-monitor" action={<Pill status={session?.status ?? 'OFFLINE'} />}>
        ...
        <div className="monitor-stats"><div><span>阶段</span><strong>{stage}</strong></div><div><span>观众数</span><strong>{queue.length + (active ? 1 : 0)}</strong></div><div><span>文案规划</span><strong>{active?.speechTargetSeconds ? `${active.speechTargetSeconds} 秒参考` : '—'}</strong></div><div><span>自然音频</span><strong>{active?.tts?.durationMs ? `${(active.tts.durationMs / 1000).toFixed(1)} 秒` : '—'}</strong></div></div>
