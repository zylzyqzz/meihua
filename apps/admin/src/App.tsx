...
export function App() {
  ...
  const changeCommentKeywords = (value: string) => settings && setSettings({ ...settings, engagement: { ...settings.engagement, commentRules: [{ ...(firstComment ?? { id: 'question-recognition', label: '问题识别', priority: 'NORMAL', matchMode: 'CONTAINS', stripKeyword: true, cooldownMinutes: 10, queueExpireMinutes: settings.queue.expireMinutes, speechTargetSeconds: settings.reading.speechTargetSeconds }), enabled: true, keywords: [...new Set(value.split(/[、,，]+/).map((item) => item.trim()).filter(Boolean))], queueExpireMinutes: settings.queue.expireMinutes, speechTargetSeconds: settings.reading.speechTargetSeconds }] } });

  const saveSimpleRules = async () => {
    if (!settings) return;
    const normalized: AppSettings = {
      ...settings,
      gifts: { ...settings.gifts, rules: settings.gifts.rules.map((rule) => ({ ...rule, priority: rule.priority === 'MANUAL' ? 'HIGH' : rule.priority, leaderboardPoints: Math.max(rule.minRepeatCount, 1) })) },
      engagement: {
        ...settings.engagement,
  ...
  const simplePage = settings && <section className="page simple-page">
    ...
    <div className="qualification-layout">
      <Panel title="资格入口" hint="先获得资格，再用一条明确问题领取资格；普通评论不会直接排队。" className="free-rules-panel"><div className="simple-rules"><label><span><Heart weight="fill" />累计点赞资格</span><input type="number" min="1" value={firstLike?.threshold ?? 100} onChange={(event) => changeLikeThreshold(Number(event.target.value))} /><small>同一观众累计达到 100 次后获得一次 30 秒测算资格。</small></label><label><span><ChatCircle weight="fill" />问题识别关键词</span><input value={firstComment?.keywords.join('、') ?? ''} onChange={(event) => changeCommentKeywords(event.target.value)} /><small>关键词只用于识别和清理问题，不会单独发资格。请提示观众：达到点赞或礼物条件后，再发送一条完整问题。</small></label></div></Panel>
      <Panel title="排队策略" hint="系统自动处理，不需要直播中手工操作。" className="queue-policy-panel"><div className="policy-fields"><label><span><ListNumbers weight="fill" />画面显示人数</span><input type="number" min="1" max="12" value={settings.queue.maxVisible} onChange={(event) => setSettings({ ...settings, queue: { ...settings.queue, maxVisible: Math.max(1, Math.min(12, Number(event.target.value))) } })} /><small>默认显示前 4 位。</small></label><label><span><ListNumbers weight="fill" />队列总容量</span><input type="number" min="1" max="100" value={settings.queue.maxTotal} onChange={(event) => setSettings({ ...settings, queue: { ...settings.queue, maxTotal: Math.max(1, Math.min(100, Number(event.target.value))) } })} /><small>默认最多 100 位。</small></label><label><span><Timer weight="fill" />资格等待时限</span><input type="number" min="1" value={settings.queue.expireMinutes} onChange={(event) => setSettings({ ...settings, queue: { ...settings.queue, expireMinutes: Math.max(1, Number(event.target.value)) } })} /><small>超时后需要重新取得资格。</small></label><label><span><SpeakerHigh weight="fill" />默认测算时长</span><select value={settings.reading.speechTargetSeconds} onChange={(event) => setSettings({ ...settings, reading: { ...settings.reading, speechTargetSeconds: Number(event.target.value) } })}>{speechDurationOptions.map((seconds) => <option key={seconds} value={seconds}>{seconds} 秒</option>)}</select><small>点赞资格和默认礼物统一为 30 秒。</small></label></div><div className="queue-flow"><b>自动执行顺序</b><span>100 次点赞或指定礼物 → 提交明确问题 → 正式排队 → 后台预处理 → 播报</span><small>没有资格的普通评论不会占用队列。</small></div></Panel>
    </div>
